...
from decisionmaking_pb2_grpc import Employee, FuelColumn, Location, Station
...
class DecisionMaking(decisionmaking_pb2_grpc.DecisionmakingServicer):
    ...
    def MakeSnapshot(self, request, context):
        ...
        month_timestamp_count = config.monthTimestampCount
        ...
        def every_month():
            # Пополняем хранилище
            snapshot.fuelStorageAmount += snapshot.fuelSupplies[month_number]
            # Платим за содержание АЗС
            snapshot.bank -= (
                config.maintenanceStationCost * len(snapshot.fuelStations)
            )

            for station in snapshot.fuelStations:
                # Платим за содержание каждой колонки на АЗС
                snapshot.bank -= (
                    config.maintenanceColumnCost * len(station.columns)
                )
                ...
                snapshot.bank -= (
                    employees['refueller'] * config.refuillerSalary
                )
                snapshot.bank -= (
                    employees['cashier'] * config.cashierSalary
                )
                snapshot.bank -= (
                    employees['director'] * config.directorSalary
                )
                snapshot.bank -= (
                    employees['security'] * config.securitySalary
                )
        ...
        def get_avg_receipt(number_of_columns):
            ...
            '''
            return config.baseAvgReceipt * config.receiptAvgCoef ** (
                number_of_columns - 1
            )
        ...
        def can_to_fill_car(station):
            ...
            conditions = (
                ...
                timestamp + config.carRefuelingTime <= year,
            )
        ...
        def can_to_build_column(station):
            ...
            conditions = (
                ...
                timestamp + config.columnBuildingTime <= year,
            )
            return all(conditions)
        ...
        def build_column(station):
            ...
            station.columns.append(
                FuelColumn(busy_to=timestamp + config.columnBuildingTime)
            )
            # Нанимаем заправщика
            ...
            number_of_cashiers = len(filter(
                lambda emp: emp.role == 'cashier', station.employees
            ))
            if len(station.columns) / number_of_cashiers > config.needAdditionalCashierColumnCount:
                station.employees.append(Employee(
                    role='cashier',
                    contract='td'
                ))
        ...
        def calculate_fuel_time(station):
            ...
            return (station.fuel_amount + future_fuel) // (
                len(station.columns) * AVG_RECEIPT_FUEL
            ) * config.carRefuelingTime
        ...
        def can_to_delivery_tanker(station):
            ...
            conditions = (
                less_time_work <= config.fuelDeliveryTime,
                len(filter(
                    lambda tank: tank.busy_to <= timestamp, snapshot.tankers
                )),
                snapshot.fuelStorageAmount >= fuel_to_delivery,
            )
        ...
        def delivery_tanker(station):
            ...
            for tanker in snapshot.tankers:
                if tanker.busy_to <= timestamp:
                    tanker.busy_to = timestamp + config.fuelDeliveryTime
                    # Считаем количество топлива, которое надо заказать
                    fuel_to_delivery = AVG_RECEIPT_FUEL * month_timestamp_count * len(
                        station.columns
                    ) // config.carRefuelingTime
        ...
        def can_to_build_station():
            ...
            # до конца года
            conditions = (
                len(snapshot.stations) < MAX_STATIONS
                timestamp + config.stationBuildingTime <= year,
            )
            return all(conditions)
        ...
        def build_station():
            snapshot.stations.append(Station(
                fuel_amount=0,
                location=get_new_location(),
                columns=0,
                busy_to=config.stationBuildingTime,
                employees=[
                    Employee(
                        role='director',
                        contract='td'
            ...
            build_column(snapshot.stations[-1])
        ...
        for tanker in snapshot.tankers:
            ...
            fuel_station = fuelStations[fuel_station_id]

            if tanker.busy_to == timestamp:
                snapshot.fuel_station.fuel_amount += tanker.fuel_amount
            else:
                x, y = get_tanker_coord(fuel_station, tanker)
                tanker.location = Location(latitude=x, longitude=y)
            ...
            if station.busy_to <= timestamp:
                ...
                for column in station.columns:
                    if column.busy_to <= timestamp and can_to_fill_car(station):
                        ...
                        column.busy_to = timestamp + config.carRefuelingTime
